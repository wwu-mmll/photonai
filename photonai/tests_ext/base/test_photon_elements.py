...
def test_CrossValidation_inner_cv():
    test_value = 3
    assert (
        Hyperpipe.CrossValidation(test_value, 1, True, 0.3, 4, True).inner_cv
        == test_value
    )
...
def test_CrossValidation_outer_cv():
    test_value = 3
    assert (
        Hyperpipe.CrossValidation(test_value, test_value, True, 0.3, 4, True).outer_cv
        == test_value
    )
...
def test_CrossValidation_eval_final_performance():
    test_value = True
    assert (
        Hyperpipe.CrossValidation(
            test_value, 1, test_value, 0.3, 4, True
...
def test_CrossValidation_test_size():
    test_value = 0.37
    assert (
        Hyperpipe.CrossValidation(test_value, 1, True, test_value, 4, True).test_size
        == test_value
    )
...
def test_CrossValidation_calculate_metrics_per_fold():
    ...
    assert (
        Hyperpipe.CrossValidation(
            test_value, 1, True, 0.3, test_value, True
...
def test_CrossValidation_inner_folds():
    test_value = dict()
    assert (
        Hyperpipe.CrossValidation(3, 1, True, 0.3, 4, test_value).inner_folds
        == test_value
    )

# 13
def test_get_package_info():  # too many args
    edi = ElementDictionary()
    assert isinstance(edi.get_package_info(), dict)
...
def test_get_package_info():  # too many args
    edi = ElementDictionary()
    len_DICT = 127
    assert len(edi.get_package_info()) == len_DICT
