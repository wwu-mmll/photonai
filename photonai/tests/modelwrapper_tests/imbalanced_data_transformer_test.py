...
from imblearn.under_sampling._prototype_selection.tests import (
    test_instance_hardness_threshold,
)
...
class ImbalancedDataTransformTest(BaseModelWrapperTest):
    ...
    def test_strategy_oversampling(self):
        ...
        imbalanced_data_transformer = ImbalancedDataTransformer(
            method_name="SMOTE",
            sampling_strategy={0: 9, 1: 12},
        ...
        X_resampled, y_resampled = imbalanced_data_transformer.fit_transform(
            test_smote.X, test_smote.Y
        )
        X_gt = np.array(
            [
    ...
    def test_strategy_undersampling(self):
        ...
        """
        imbalanced_data_transformer = ImbalancedDataTransformer(
            method_name="InstanceHardnessThreshold",
        ...
        X_resampled, y_resampled = imbalanced_data_transformer.fit_resample(
            test_instance_hardness_threshold.X, test_instance_hardness_threshold.Y
        )
    ...
    def test_strategy_combine(self):
        ...
        """
        imbalanced_data_transformer = ImbalancedDataTransformer(
            method_name="SMOTETomek", random_state=test_smote_tomek.RND_SEED
        )
        X_resampled, y_resampled = imbalanced_data_transformer.fit_resample(
            test_smote_tomek.X, test_smote_tomek.Y
        )
        X_gt = np.array(
            [
