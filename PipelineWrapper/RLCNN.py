...
class RLCNN(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, data, targets):
        ...
        W_conv1 = RLCNN.weight_variable([patch_size, patch_size, 1, num_features])
        b_conv1 = RLCNN.bias_variable([num_features])
        ...
        h_pool1 = RLCNN.max_pool_2x2(h_conv1, reduction_1)

        # second convolution
        # compute 64 features for each 5x5 patch.
        W_conv2 = RLCNN.weight_variable([patch_size, patch_size, num_features, 2*num_features])
        b_conv2 = RLCNN.bias_variable([2*num_features])
        h_conv2 = tf.nn.relu(RLCNN.conv2d(h_pool1, W_conv2) + b_conv2)
        # reduce from 14x14 to 7x7
        h_pool2 = RLCNN.max_pool_2x2(h_conv2, reduction_2)

        # densely connected layer
        # weights = 7x7 image and 64 features, 1024 neurons
        final_size = (image_width/reduction_1/reduction_2) * (image_height/reduction_1/reduction_2) * 2 * num_features
        W_fc1 = RLCNN.weight_variable([final_size, densely_neurons])
        b_fc1 = RLCNN.bias_variable([densely_neurons])
    ...
    def predict(self, data):
        return self.model.y_conv(feed_dict={
            self.x: data})
