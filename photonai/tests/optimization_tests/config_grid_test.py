...
class CreateGlobalConfigBaseElements(unittest.TestCase):
    ...
    def create_global_config_one_element_test(self):
        ...
        self.assertDictEqual(
            create_global_config_dict([self.pca]), {"PCA__n_components": [1, 2]}
        )
        self.assertDictEqual(
            create_global_config_dict([self.rf]), {}
        )  # no hyperparameter
        ...
        self.assertListEqual(
            ...
            [{"StandardScaler__disabled": False}, {"StandardScaler__disabled": True}],
        )
        self.assertListEqual(
            create_global_config_grid([self.pca]),
            [{"PCA__n_components": 1}, {"PCA__n_components": 2}],
        )
        self.assertListEqual(
            create_global_config_grid([self.rf]), [{}]
        )  # no hyperparameter
...
class CreateGlobalConfigAdvancedElements(PhotonBaseTest):
    def setUp(self):
        ...
        self.switch_in_switch = Switch(
            "Switch_in_switch", [self.branch, self.pipe_switch]
        )
    ...
    def create_global_config_switch_test(self):
        ...
        # config grid
        self.assertListEqual(
            create_global_config_grid([self.pipe_switch]),
            [
    ...
    def create_global_config_branch_test(self):
        ...
        self.assertDictEqual(
            ...
            {"branch__SVC__C": [0.1, 1], "branch__SVC__kernel": ["rbf", "sigmoid"]},
        )
        ...
        self.assertListEqual(
            ...
            [
                {"branch__SVC__C": 0.1, "branch__SVC__kernel": "rbf"},
    ...
    def create_global_config_switch_in_swicht_test(self):
        ...
        # config dict
        self.assertDictEqual(
            create_global_config_dict([self.switch_in_switch]),
            {
                "Switch_in_switch__current_element": [
                    (0, 0),
                    (0, 1),
                    (0, 2),
                    (0, 3),
        ...
        # config grid
        self.assertListEqual(
            create_global_config_grid([self.switch_in_switch]),
            [
                {"Switch_in_switch__current_element": (0, 0)},
