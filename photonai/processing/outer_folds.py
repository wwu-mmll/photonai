...
class OuterFoldManager:
    ...
    def _prepare_optimization(self):
        ...
        self.optimizer.prepare(
            pipeline_elements, self.optimization_info.maximize_metric
        )
        ...
        if self.optimization_info.performance_constraints is not None:
            if isinstance(self.optimization_info.performance_constraints, list):
                self.constraint_objects = [
                    original.copy_me()
                    for original in self.optimization_info.performance_constraints
                ]
            else:
                self.constraint_objects = [
    ...
    def _generate_inner_folds(self):
        ...
        self.inner_folds = FoldInfo.generate_folds(
            self.cross_validaton_info.inner_cv,
            self._validation_X,
            self._validation_y,
            self._validation_kwargs,
        )

        self.cross_validaton_info.inner_folds[self.outer_fold_id] = {
            f.fold_id: f for f in self.inner_folds
        }
