...
class NeuroTest(PhotonBaseTest):
    def setUp(self):
        ...
        self.atlas_folder = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../../neuro/atlases/"
        )
        self.atlas_name = "AAL"
        self.roi_list = ["Hippocampus_R", "Hippocampus_L", "Amygdala_L", "Amygdala_R"]
        self.X = AtlasLibrary().get_nii_files_from_folder(
            self.test_folder, extension=".nii"
        )
    ...
    def test_brain_atlas_load(self):
        ...
        man_map = image.load_img(
            os.path.join(self.atlas_folder, "AAL_SPM12/AAL.nii.gz")
        ).get_data()
    ...
    def test_brain_masker(self):
        ...
        roi_objects = BrainAtlas._get_rois(
            atlas_obj, which_rois=self.roi_list, background_id=0
        )

        for roi in roi_objects:
            masker = BrainMask(
                mask_image=roi, affine=affine, shape=shape, extract_mode="vec"
            )
            own_calculation = masker.transform(self.X[0])
            nilearn_func = NiftiMasker(
                mask_img=roi.mask,
    ...
    def test_resampling_and_smoothing(self):
        ...
        create_instances_and_transform(
            "SmoothImages", smoothing_param_dict, nilearn_smoothed_X
        )
        ...
        nilearn_resampled_X = []
        for element in self.X:
            nilearn_resampled_X.append(
                image.resample_img(element, **resample_param_dict)
            )
        create_instances_and_transform(
            "ResampleImages", {"voxel_size": [5, 5, 5]}, nilearn_resampled_X
        )
    ...
    def test_neuro_module_branch(self):
        ...
        nr_files_in_folder = len(
            glob.glob(os.path.join(nmb.base_element.cache_folder, "*.p"))
        )
        self.assertTrue(nr_files_in_folder == 3 * nr_niftis)
        self.assertTrue(
            len(nmb.base_element.cache_man.cache_index.items()) == (3 * nr_niftis)
        )
        ...
        self.assertTrue(
            len(nmb.base_element.cache_man.cache_index.items()) == (3 * len(self.X))
        )
    ...
    def test_custom_mask(self):
        ...
        mask = PipelineElement(
            "BrainMask", mask_image=custom_mask, extract_mode="vec", batch_size=20
        )
        X_masked = mask.transform(self.X)

        with self.assertRaises(FileNotFoundError):
            mask = PipelineElement(
                "BrainMask", mask_image="XXXXX", extract_mode="vec", batch_size=20
            )
    ...
    def test_custom_atlas(self):
        ...
        atlas = PipelineElement(
            "BrainAtlas", atlas_name=custom_atlas, extract_mode="vec", batch_size=20
        )
        X_masked = atlas.transform(self.X)

        with self.assertRaises(FileNotFoundError):
            atlas = PipelineElement(
                "BrainAtlas", atlas_name="XXXXX", extract_mode="vec", batch_size=20
            )
