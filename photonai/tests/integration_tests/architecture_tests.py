...
class TestArchitectures(PhotonBaseTest):
    ...
    def setUpClass(cls) -> None:
        ...
        if cls.test_multiple_hyperpipes:
            ...
            outer_cv_list = [
                None,
                KFold(n_splits=3, shuffle=True),
                ShuffleSplit(n_splits=1, test_size=0.25),
                LeaveOneOut(),
            ]
            ...
            for (
                ...
                cls.hyperpipes.append(
                    cls.create_hyperpipes(
                        plots=plots_bool,
                        optimizer=optimizer,
                        inner_cv=inner_cv,
                        outer_cv=outer_cv,
        ...
        cls.classification_data = make_classification(
    ...
    def add_metrics(pipe, analysis_type):
        if analysis_type == "classification":
            ...
            raise NotImplementedError(
                "Only regression and classification is supported at the moment."
            )
    ...
    def run_hyperpipe(self, pipe, analysis_type):
        ...
        if analysis_type == "classification":
            pipe.fit(
                self.classification_data[0],
                self.classification_data[1],
                **{"groups": self.groups, "cov1": self.cov1, "cov2": self.cov2}
            )

        elif analysis_type == "regression":
            pipe.fit(
                self.regression_data[0],
                self.regression_data[1],
                **{"groups": self.groups, "cov1": self.cov1, "cov2": self.cov2}
            )
    ...
    def test_regression_1(self):
        for original_hyperpipe in self.hyperpipes:
            ...
            pipe += PipelineElement(
                name="SVR", hyperparameters={"kernel": Categorical(["linear", "rbf"])}
            )
    ...
    def test_regression_2(self):
        for original_hyperpipe in self.hyperpipes:
            ...
            switch += PipelineElement(
                ...
                hyperparameters={
                    "min_samples_split": FloatRange(
                        start=0.05, step=0.1, stop=0.26, range_type="range"
                    )
                },
            )
            pipe += switch
    ...
    def test_regression_4(self):
        for original_hyperpipe in self.hyperpipes:
            ...
            my_switch += PipelineElement(
                ...
                hyperparameters={
                    "percentile": IntegerRange(
                        start=5, step=20, stop=66, range_type="range"
                    )
                },
                test_disabled=True,
            )
    ...
    def test_regression_8(self):
        for original_hyperpipe in self.hyperpipes:
            ...
            source1_branch += DataFilter(
                indices=np.arange(start=0, stop=int(np.floor(self.X_shape[1] / 2)))
            )
            ...
            source2_branch += DataFilter(
                indices=np.arange(
                    start=int(np.floor(self.X_shape[1] / 2)), stop=self.X_shape[1]
                )
            )
    ...
    def test_classification_1(self):
        for original_hyperpipe in self.hyperpipes:
            ...
            pipe += PipelineElement(
                name="SVC", hyperparameters={"kernel": Categorical(["linear", "rbf"])}
            )
    ...
    def test_classification_11(self):
        for original_hyperpipe in self.hyperpipes:
            ...
            # add to pipe
            pipe += Stack(
                "estimator_stack", elements=[SVC1, SVC2, RF], use_probabilities=True
            )
            pipe += PipelineElement("RandomForestClassifier")
