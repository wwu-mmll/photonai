...
class InnerFoldTests(PhotonBaseTest):
    def setUp(self):
        ...
        self.cross_validation.inner_folds = {
            self.outer_fold_id: {
                ...
                for i, (train, test) in enumerate(self.inner_cv.split(self.X, self.y))
            }
        }
        self.optimization = Hyperpipe.Optimization(
            "grid_search", {}, ["accuracy", "recall", "specificity"], "accuracy", None
        )
    ...
    def test_fit_against_sklearn(self):
        ...
        for fold_obj in self.cross_validation.inner_folds[self.outer_fold_id].values():
            ...
            self.assertTrue(
                np.array_equal(sklearn_predictions, photon_test_results.y_pred)
            )
    ...
    def test_save_predictions(self):
        ...
        for inner_fold in config_item.inner_folds:
            self.assertEqual(
                len(inner_fold.training.y_pred), inner_fold.number_samples_training
            )
            self.assertEqual(
    ...
    def test_process_fit_results(self):
        ...
        def assert_fold_operations(expected_operations, returned_metric_list):
            ...
            returned_formatted_metric_list = [
                m.metric_name + "__" + str(m.operation) for m in returned_metric_list
            ]
            self.assertTrue(
        ...
        self.assertTrue(
            len(across_and_per_folds_config_item.metrics_train)
            == 2 * num_of_metrics + num_of_metrics
        )
        self.assertTrue(
            len(across_and_per_folds_config_item.metrics_test)
            == 2 * num_of_metrics + num_of_metrics
        )
        ...
        assert_fold_operations(
            [FoldOperations.RAW], across_folds_config_item.metrics_train
        )
        assert_fold_operations(
            [FoldOperations.RAW], across_folds_config_item.metrics_test
        )
        ...
        assert_fold_operations(
            [FoldOperations.MEAN, FoldOperations.STD], per_fold_config_item.metrics_test
        )
