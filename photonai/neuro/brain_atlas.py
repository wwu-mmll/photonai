...
class AtlasLibrary:
    ...
    MASK_DICTIONARY = {
        "MNI_ICBM152_GrayMatter": "mni_icbm152_gm_tal_nlin_sym_09a.nii.gz",
    ...
    def _load_photon_atlases(self):
        ...
        for atlas_id, atlas_info in self.ATLAS_DICTIONARY.items():
            atlas_file = glob.glob(path.join(dir_atlases, path.join("*", atlas_info)))[
                0
            ]
            atlas_basename = path.basename(atlas_file)[:-7]
            atlas_dir = path.dirname(atlas_file)
            photon_atlases[atlas_id] = AtlasObject(
                name=atlas_id,
                path=atlas_file,
    ...
    def _add_atlas_to_library(
        self, atlas_name, target_affine=None, target_shape=None, mask_threshold=None
    ):
        ...
        # check labels
        if Path(
            atlas_object.labels_file
        ).is_file():  # if we have a file with indices and labels
            labels = pd.read_table(atlas_object.labels_file, header=None)
            labels_dict = pd.Series(
                labels.iloc[:, 1].values, index=labels.iloc[:, 0]
            ).to_dict()
            ...
            if not sorted(atlas_object.indices) == sorted(list(labels_dict.keys())):
                ...
                {}
                """.format(
                        str(sorted(self.indices)), str(sorted(list(labels_dict.keys())))
                    )
                )

                atlas_object.roi_list = [
                    RoiObject(index=i, label=str(i), size=np.sum(i == atlas_object.map))
        ...
        for roi in atlas_object.roi_list:
            ...
            roi.mask = image.new_img_like(
                atlas_object.path, atlas_object.map == roi.index
            )
        ...
        self.library[
            (atlas_name, str(target_affine), str(target_shape), str(mask_threshold))
        ] = atlas_object
    ...
    def _add_mask_to_library(
        ...
        mask_object = MaskObject(
            name=mask_name, mask_file=original_mask_object.mask_file
        )

        # mask_object.mask = image.threshold_img(mask_object.mask_file, threshold=mask_threshold)
        mask_object.mask = math_img(
            "img > {}".format(mask_threshold), img=mask_object.mask_file
        )

        if target_affine is not None and target_shape is not None:
            mask_object.mask = self._resample(
                mask_object.mask, target_affine=target_affine, target_shape=target_shape
            )

        # check if roi is empty
        if np.sum(mask_object.mask.dataobj != 0) == 0:
            logger.error(
                "No voxels in mask after resampling (" + mask_object.name + ")."
            )
    ...
    def get_atlas(
        ...
        if (
            ...
            self._add_atlas_to_library(
                atlas_name, target_affine, target_shape, mask_threshold
            )
    ...
    def get_mask(
        self, mask_name, target_affine=None, target_shape=None, mask_threshold=0.5
    ):
        if (mask_name, str(target_affine), str(target_shape)) not in self.library:
            self._add_mask_to_library(
                mask_name, target_affine, target_shape, mask_threshold
            )

        return self.library[
            (mask_name, str(target_affine), str(target_shape), str(mask_threshold))
        ]
...
class BrainAtlas(BaseEstimator):
    ...
    def transform(self, X, y=None, **kwargs):
        ...
        # get ROI mask
        atlas_obj = AtlasLibrary().get_atlas(
            self.atlas_name, self.affine, self.shape, self.mask_threshold
        )
        roi_objects = self._get_rois(
            atlas_obj, which_rois=self.rois, background_id=self.background_id
        )
        ...
        series = _utils.as_ndarray(
            _safe_get_data(X), dtype="float32", order="C", copy=True
        )
        mask_indices = list()
    ...
    def _validity_check_roi_extraction(
        self, X, y=None, filename="validity_check.nii", **kwargs
    ):
        ...
        if isinstance(which_rois, str):
            ...
            if isinstance(which_rois[0], str):
                if which_rois[0].lower() == "all":
                    return [
                        roi for roi in atlas_obj.roi_list if roi.index != background_id
                    ]
                else:
                    return AtlasLibrary().find_rois_by_label(atlas_obj, which_rois)
            else:
...
class BrainMask(BaseEstimator):
    def __init__(
        self,
        mask_image="MNI_ICBM152_WholeBrain",
        affine=None,
        shape=None,
        mask_threshold=0.5,
        extract_mode="vec",
    ...
    def transform(self, X, y=None, **kwargs):
        ...
        if isinstance(self.mask_image, str):
            self.mask_image = AtlasLibrary().get_mask(
                self.mask_image, self.affine, self.shape, self.mask_threshold
            )
        ...
        if not self.mask_image.is_empty:
            ...
            if single_roi is not None:
                if self.extract_mode == "vec":
                    ...
                    logger.error(
                        "Currently there are no other methods than 'vec', 'mean', and 'box' supported!"
                    )
            else:
                if isinstance(X, str):
                    ...
                    logger.error(
                        "Extracting ROI failed for nifti image obj. Cannot trace back path of failed file."
                    )
        else:
            logger.error(
                "Skipping self.mask_image "
                + self.mask_image.label
                + " because it is empty."
            )
    ...
    def inverse_transform(self, X, y=None, **kwargs):
        if not self.extract_mode == "vec":
            raise NotImplementedError(
                "BrainMask extract_mode={} is not supported with inverse_transform".format(
                    self.extract_mode
                )
            )
