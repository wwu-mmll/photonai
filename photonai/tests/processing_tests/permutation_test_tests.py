...
class PermutationTestTests(PhotonBaseTest):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.hyperpipe = Hyperpipe(
            "permutation_test_pipe",
    ...
    def test_get_duration_per_n(self):
        returned_duration = PermutationTest.estimated_duration_per_permutation(
            str(self.perm_id)
        )
        cached_duration = (
            self.hyperpipe.results.computation_end_time
            - self.hyperpipe.results.computation_start_time
        )
        self.assertEqual(
            round(returned_duration.total_seconds(), 2),
            round(cached_duration.total_seconds(), 2),
        )

    def test_validate_usability(self):
        self.assertTrue(
            PermutationTest.validate_permutation_test_usability(self.wizard_obj_id)
        )
        # Todo setup case where it is false

    def test_find_reference(self):
        obj_id = self.hyperpipe.results._id
        returned_obj_id = PermutationTest.find_reference(
            self.hyperpipe.output_settings.mongodb_connect_url, str(self.perm_id), False
        )._id
        self.assertEqual(obj_id, returned_obj_id)

        wizard_obj_id = str(self.wizard_obj_id)
        latest_item = PermutationTest.find_reference(
            self.hyperpipe.output_settings.mongodb_connect_url,
            ObjectId(wizard_obj_id),
            True,
        )
    ...
    def test_update_perm_id_for_wizard(self):
        ...
        my_handler.load_from_mongodb(
            self.hyperpipe.output_settings.mongodb_connect_url, str(wizard_id)
        )
        self.assertEqual(
            my_handler.results.permutation_id,
            PermutationTest.get_mother_permutation_id(str(perm_id)),
        )
