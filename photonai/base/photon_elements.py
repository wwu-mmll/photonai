...
class ParallelBranch(Branch):
    """
        An advanced substream of elements that are encapsulated into a single block of PipelineElements and perform
        parallelization.
        Parameters
        ----------
        * `name` [str]:
            Name of the ParallelBranch
        * `nr_of_processes` [int]:
            Count of processes branch is running on
        """

    def __init__(self, name: str, elements: list = None, nr_of_processes: int = 1):
        Branch.__init__(self, name, elements=elements)
    ...
    def fit(self, X, y=None, **kwargs):
        return super(ParallelBranch, self).fit(X, y, **kwargs)
    ...
    def apply_transform_parallelized(self, X):
        ...
        if self.nr_of_processes > 1:
            ...
            logger.info('ParallelBranch ' + self.name +
                        ': Using ' + str(self.nr_of_processes) + ' cores calculating ' +
