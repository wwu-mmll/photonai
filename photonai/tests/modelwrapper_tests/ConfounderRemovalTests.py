...
class ConfounderRemovalTests(PhotonBaseTest):
    def setUp(self):
        ...
        for i in range(self.X_train.shape[1]):
            # fit
            model = sm.OLS(
                endog=np.squeeze(self.X_train[:, i]), exog=ols_confounder_train
            ).fit()
            # transform
    ...
    def test_standardize_covariates(self):
        ...
        np.testing.assert_array_almost_equal(
            X_transformed[0], self.X_transformed_standardized
        )
    ...
    def test_dimensions(self):
        with self.assertRaises(ValueError):
            self.cr.fit(
                self.X, self.y, confounder=np.random.randn(self.X.shape[0] - 10, 2)
            )

    def test_key_error(self):
        with self.assertRaises(KeyError):
            self.cr.fit(
                self.X, self.y, covariate=np.random.randn(self.X.shape[0] - 10, 2)
            )
