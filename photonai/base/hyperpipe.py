...
class OutputSettings:
    ...
    def __init__(self,
        ...
        Arguments:
            mongodb_connect_url:
                Mongodb connection string that specifies a database for storing the results
            save_output:
                If True, PHOTONAI writes a summary_file, the results of the hyperparameter optimization,
                the best model and the console output to the filesystem into the given project folder.
            plots:
                If True, PHOTONAI generates some plots
            overwrite_results:
                If True, PHOTONAI writes always in the same output folder, without appending the datetime
            project_folder:
                The output folder in which all files generated by the PHOTONAI project are saved to.
            result_file_mode:
                The possible save_modes for .photon file. Possible options are:

                    - 'best' to save only the predictions of the best configuration for every outer fold
                    - 'all' to save all predictions or
                    - 'None' to not save any predictions at all.

            user_id:
                The user name of the according PHOTONAI Wizard login
            wizard_object_id:
                The object id to map the designed pipeline in the PHOTONAI Wizard to the results in the
                PHOTONAI CORE Database
            wizard_project_name:
                How the project is titled in the PHOTONAI Wizard.
                [Link to PipelineElement](photonai.base.PipelineElement)
        """
