...
class FeatureEncoderTests(unittest.TestCase):
    ...
    def test_transform(self):
        self.ordinal_encoder.fit(self.X, self.values)
        result = self.ordinal_encoder.transform(
            np.array([["a", 1, "b"], ["a", 1, "b"], ["a", 1, "b"]], dtype=object)
        )
        assert_array_equal(
            result, np.array([[0, 1, 0], [0, 1, 0], [0, 1, 0]], dtype=object)
        )

    def test_inverse_transform(self):
        self.ordinal_encoder.fit(self.X, self.values)
        result = self.ordinal_encoder.inverse_transform(
            np.array([[0, 1, 0], [0, 1, 0], [0, 1, 0]], dtype=object)
        )
        assert_array_equal(
            result,
            np.array([["a", 1, "b"], ["a", 1, "b"], ["a", 1, "b"]], dtype=object),
    ...
    def test_fit_transform(self):
        ...
        assert_array_equal(
            result, np.array([[0, 3, 0], [1, 5, 0], [0, 3, 1]], dtype=object)
        )

        result = self.ordinal_encoder.fit_transform(self.intX)
        assert_array_equal(result, self.intX)

        result = self.ordinal_encoder.fit_transform(
            np.array([["a", 1, "b"], ["a", 1, "b"], ["a", 1, "b"]], dtype=object)
        )
        assert_array_equal(
            result, np.array([[0, 1, 0], [0, 1, 0], [0, 1, 0]], dtype=object)
