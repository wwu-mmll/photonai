...
class PlotlyPlot:
    ...
    def to_plot(self) -> str:
        ...
        for i, item in enumerate(self.traces):
            ...
            result += (
                "var trace_{}".format(i) + " = { x: [" + item.get_x_to_string() + "]"
            )
        ...
        result += str(
            "var layout = {margin: {l: 200}, hovermode: 'closest', title: '"
            + str(self.title)
            + "'"
        )
    ...
    def to_error_plot(self) -> str:
        ...
        for item in self.traces:
            result += (
                "var " + item.variable_name + " = { x: [" + item.get_x_to_string() + "]"
            )
            result += ", y: [" + item.get_y_to_string() + "]"
            result += ", type: '" + item.trace_type + "'"
            result += ", mode: '" + item.mode + "'"
...
class PiePlotlyPlot(PlotlyPlot):
    def __init__(
        self, plot_name: str, title: str, traces: list = None, show_legend: bool = True
    ):

        super(PiePlotlyPlot, self).__init__(
            plot_name=plot_name, title=title, traces=traces, show_legend=show_legend
        )
        self.result_dict = {
            "layout": {
