...
class PhotonFoldsTests(unittest.TestCase):
    ...
    def base_assertions(self, cv, nr_of_folds, eval_final_performance=True):
        ...
        for generated_fold in fold_list:
            self.assertEqual(
                len(generated_fold.train_indices),
                (nr_of_folds - 1) * (self.num_subjects / nr_of_folds),
            )
            self.assertEqual(
                len(generated_fold.test_indices), (self.num_subjects / nr_of_folds)
            )
    ...
    def test_group_k_fold_generation(self):
        ...
        for generated_fold in fold_list:
            train_groups = np.unique(
                self.kwargs["groups"][generated_fold.train_indices]
            )
    ...
    def test_no_cv_strategy(self):
        ...
        self.assertEqual(
            len(fold_list[0].train_indices), (1 - test_size) * self.num_subjects
        )
