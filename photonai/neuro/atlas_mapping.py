...
class AtlasMapper:
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        with open(
            os.path.join(
                self.folder, self.original_hyperpipe_name + "_atlas_mapper_meta.json"
            ),
            "w",
        ) as fp:
        ...
        df.to_csv(
            os.path.join(
                self.folder, self.original_hyperpipe_name + "_atlas_mapper_results.csv"
            )
        )
    ...
    def surface_plots(self, perf_img):
        ...
        figure, axes = plt.subplots(
            2, 2, subplot_kw={"projection": "3d"}, figsize=(12, 12)
        )
        axes = axes.ravel()
    ...
    def predict(self, X, **kwargs):
        if len(self.hyperpipes_to_fit) == 0:
            raise Exception(
                "No hyperpipes to predict. Did you remember to fit or load the Atlas Mapper?"
            )
    ...
    def load_from_folder(self, folder: str, analysis_name: str = None):
        ...
        if analysis_name:
            meta_file = glob(
                os.path.join(folder, analysis_name + "_atlas_mapper_meta.json")
            )
        else:
        ...
        if len(meta_file) == 0:
            raise FileNotFoundError(
                "Couldn't find atlas_mapper_meta.json file in {}. Did you specify the correct analysis name?".format(
                    folder
                )
            )
        elif len(meta_file) > 1:
            raise ValueError(
                "Found multiple atlas_mapper_meta.json files in {}".format(folder)
            )
    ...
    def _load(self, file):
        ...
        for roi_name, infos in self.hyperpipe_infos.items():
            model_path = os.path.join(
                os.path.join(self.folder, infos["hyperpipe_name"] + "_results"),
