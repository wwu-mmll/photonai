...
@application.route(
    "/pipeline/<storage>/<name>/outer_fold/<fold_nr>/compare_configurations",
    methods=["POST"],
)
def compare_configurations(storage, name, fold_nr):
    try:
        ...
        for config in outer_fold.tested_config_list:
            if str(config.config_nr) in selected_config_nr_list:
                ...
                trace_training = PlotlyTrace(
                    "config_" + str(config.config_nr), "", "bar"
                )
        ...
        plot_training = PlotlyPlot(
            "comparison_metrics_training",
            "train metrics of selected configurations",
            trace_training_list,
        )
        plot_test = PlotlyPlot(
            "comparison_metrics_test",
            "test metrics of selected configurations",
            trace_test_list,
        )

        return render_template(
            "configuration/compare.html",
            pipe=pipe,
