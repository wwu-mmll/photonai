...
from photonai.processing.results_structure import (
    MDBPermutationResults,
    MDBPermutationMetrics,
    MDBHyperpipe,
    FoldOperations,
)
...
class PermutationTest:
    def __init__(
        ...
        self.mother_permutation_id = PermutationTest.get_mother_permutation_id(
            permutation_id
        )
    ...
    def manage_metrics(metrics, last_element=None, best_config_metric=""):
        ...
        for metric in metrics:
            metric_dict[metric] = {
                "name": metric,
                "greater_is_better": PermutationTest.set_greater_is_better(
                    metric, last_element
                ),
            }
        if best_config_metric not in metric_dict.keys():
            metric_dict[best_config_metric] = {
                "name": best_config_metric,
                "greater_is_better": PermutationTest.set_greater_is_better(
                    best_config_metric
                ),
            }
    ...
    def fit(self, X, y, **kwargs):
        ...
        self.metrics = PermutationTest.manage_metrics(
            self.pipe.optimization.metrics, self.pipe.elements[-1], best_config_metric
        )
        ...
        try:
            existing_reference = MDBHyperpipe.objects.raw(
                {
                    "permutation_id": self.mother_permutation_id,
        ...
        existing_permutations = list(
            MDBHyperpipe.objects.raw(
                {"permutation_id": self.permutation_id, "computation_completed": True}
            ).only("name")
        )
        existing_permutations = [
            int(perm_run.name.split("_")[-1]) for perm_run in existing_permutations
        ]
        ...
        performance_df.to_csv(
            os.path.join(
                existing_reference.output_folder, "permutation_test_results.csv"
            )
        )
        return self

    @staticmethod
    def run_parallelized_permutation(
        hyperpipe_constructor,
        X,
        perm_run,
        y_perm,
    ...
    def _calculate_results(
        ...
        try:
            ...
            all_permutations = list(
                MDBHyperpipe.objects.raw(
                    {"permutation_id": permutation_id, "computation_completed": True}
            ...
            metrics = PermutationTest.manage_metrics(
                metric_list, None, mother_permutation.hyperpipe_info.best_config_metric
            )
            ...
            result = PermutationTest.PermutationResult(
                true_performances, perm_performances, p, number_of_permutations, n_perms
            )
        ...
        def _find_mummy(permutation_id):
            if not find_wizard_id:
                return (
                    MDBHyperpipe.objects.raw(
                        {
                            "permutation_id": PermutationTest.get_mother_permutation_id(
                                permutation_id
                            ),
                            "computation_completed": True,
                        }
                    )
                    .order_by([("computation_start_time", DESCENDING)])
                    .first()
                )
            else:
                return (
                    MDBHyperpipe.objects.raw({"wizard_object_id": permutation_id})
                    .order_by([("computation_start_time", DESCENDING)])
                    .first()
                )
    ...
    @staticmethod
    def prepare_for_wizard(
        permutation_id,
        wizard_id,
        mongo_db_connect_url="mongodb://trap-umbriel:27017/photon_results",
    ):
        mother_permutation = PermutationTest.find_reference(
            mongo_db_connect_url, permutation_id=wizard_id, find_wizard_id=True
        )
        mother_permutation.permutation_id = PermutationTest.get_mother_permutation_id(
            permutation_id
        )
        mother_permutation.save()
        result = dict()
        result["estimated_duration"] = (
            mother_permutation.computation_end_time
            - mother_permutation.computation_start_time
        )
    ...
    def __validate_usability(mother_permutation):
        if mother_permutation is not None:
            if mother_permutation.dummy_estimator:
                ...
                dummy_threshold_to_beat = [
                    i.value
                    for i in mother_permutation.dummy_estimator.test
                    if i.metric_name == best_config_metric
                    and i.operation == str(FoldOperations.MEAN)
                ]
                if len(dummy_threshold_to_beat) > 0:
                    ...
                    if mother_permutation.hyperpipe_info.maximize_best_config_metric:
                        if (
                            mother_permutation.best_config.best_config_score.validation.metrics[
                                best_config_metric
                            ]
                            > dummy_threshold_to_beat
                        ):
    ...
    def calculate_p(true_performance, perm_performances, metrics, n_perms):
        ...
        for _, metric in metrics.items():
            if metric["greater_is_better"]:
                p[metric["name"]] = np.sum(
                    true_performance[metric["name"]]
                    < np.asarray(perm_performances[metric["name"]])
                ) / (n_perms + 1)
            else:
                p[metric["name"]] = np.sum(
                    true_performance[metric["name"]]
