...
def show_outer_fold(storage, name, fold_nr):
    try:
        ...
        for (
            key,
            value,
        ) in outer_fold.best_config.best_config_score.training.metrics.items():
        ...
        for (
            key,
            value,
        ) in outer_fold.best_config.best_config_score.validation.metrics.items():
            metric = Metric(key, value)
        ...
        for (
            metric
        ) in outer_fold.best_config.best_config_score.validation.metrics.keys():
            best_config_plot = PlotlyPlot(
                "fold_performance_" + metric,
                metric.replace("_", " "),
                show_legend=False,
                margin={"r": 30, "l": 30},
            )
        ...
        if not pipe.hyperpipe_info.eval_final_performance:
            ...
            if pipe.hyperpipe_info.estimation_type == "classifier":
                final_value_training_plot = plotly_confusion_matrix(
                    "best_config_training_values",
                    "Confusion Matrix Train",
                    [[y_true, y_pred]],
                )
            else:
                final_value_training_plot = plot_scatter(
                    [[y_true, y_pred]],
            ...
            if pipe.hyperpipe_info.estimation_type == "classifier":
                final_value_validation_plot = plotly_confusion_matrix(
                    "best_config_validation_values",
                    "Confusion Matrix Test",
                    [[y_true, y_pred]],
                )
            else:
                # START building final values for validation set (best config)
                final_value_validation_plot = plot_scatter(
                    [[y_true, y_pred]],
        ...
        for config in outer_fold.tested_config_list:
            config_dict = Config(
                "config_" + str(config.config_nr), config_nr=config.config_nr
            )

            error_plot_train = PlotlyPlot(
                "train_config_" + str(config.config_nr),
                "Inner Training Performance",
                [],
                show_legend=False,
            )
            error_plot_test = PlotlyPlot(
                "test_config_" + str(config.config_nr),
            ...
            for inner_fold in config.inner_folds:
                trace_training_metrics = PlotlyTrace(
                    "training_fold_" + str(inner_fold.fold_nr),
                    "markers",
                    "scatter",
                    trace_color="rgb(91, 91, 91)",
                )
                trace_test_metrics = PlotlyTrace(
                    "test_fold_" + str(inner_fold.fold_nr),
                    "markers",
                    "scatter",
                    trace_color="rgb(91, 91, 91)",
                )
            ...
            trace_training = PlotlyTrace(
                "training_mean_" + str(config.config_nr),
                "markers",
                "scatter",
                trace_size=8,
                with_error=True,
            )
            trace_test = PlotlyTrace(
                "test_mean_" + str(config.config_nr),
                "markers",
                "scatter",
                trace_size=8,
                with_error=True,
