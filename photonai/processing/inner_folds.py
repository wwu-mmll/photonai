...
from photonai.helper.helper import (
    PhotonPrintHelper,
    PhotonDataHelper,
...
class InnerFoldManager(object):
    ...
    def fit(self, X, y, **kwargs):
        ...
        try:
            ...
            for idx, (inner_fold_id, inner_fold) in enumerate(
                self.cross_validation_infos.inner_folds[self.outer_fold_id].items()
            ):
                ...
                if not config_item.human_readable_config:
                    ...
                    logger.clean_info(
                        json.dumps(
                            config_item.human_readable_config, indent=4, sort_keys=True
                        )
                    )
                ...
                job_data = InnerFoldManager.InnerCVJob(
                    ...
                    test_data=InnerFoldManager.JobData(
        ...
        logger.debug(
            json.dumps(config_item.human_readable_config, indent=4, sort_keys=True)
        )
    ...
    def process_fit_results(
        config_item, calculate_metrics_across_folds, calculate_metrics_per_fold, metrics
    ):
        ...
        for fold in config_item.inner_folds:
            ...
            if calculate_metrics_across_folds:
                ...
                metrics_train = Scorer.calculate_metrics(
                    overall_y_true_train, overall_y_pred_train, metrics_to_calculate
                )
                metrics_test = Scorer.calculate_metrics(
                    overall_y_true_test, overall_y_pred_test, metrics_to_calculate
                )
    ...
    def fit_and_score(job: InnerCVJob):
        ...
        # score test data
        curr_test_fold = InnerFoldManager.score(
            pipe,
            job.test_data.X,
            job.test_data.y,
            job.metrics,
