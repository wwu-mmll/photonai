...
class RegistryTest(PhotonBaseTest):
    def setUp(self):
        ...
        self.test_directory = os.path.join(
            os.path.dirname(os.path.abspath(__file__)).split("test")[0], "test"
        )
        self.custom_folder = os.path.join(
            self.test_directory, "base_tests/custom_elements/"
        )
    ...
    def test_python_file_not_in_custom_folder(self):
        with self.assertRaises(FileNotFoundError):
            self.registry.register(
                "SomeName", "not_existing_file.SomeName", "Estimator"
            )
    ...
    def test_register_element(self):
        with self.assertRaises(ValueError):
            self.registry.register(
                "MyCustomEstimator", "custom_estimator.CustomEstimator", "WrongType"
            )

        self.registry.register(
            "MyCustomEstimator", "custom_estimator.CustomEstimator", "Estimator"
        )
