...
class ResultHandlerAndHelperTests(PhotonBaseTest):
    ...
    def test_cv_config_and_dummy_nr(self):
        ...
        self.assertTrue(
            len(self.hyperpipe.cross_validation.outer_folds) == self.outer_fold_nr
        )
    ...
    def check_for_dummy(self):
        ...
        if self.hyperpipe.cross_validation.eval_final_performance:
            self.assertTrue(
                len(self.hyperpipe.results.dummy_estimator.test)
                == expected_dummy_metrics
            )
        # we should have mean and std for each metric respectively
        self.assertTrue(
            len(self.hyperpipe.results.dummy_estimator.train) == expected_dummy_metrics
        )
    ...
    def test_get_predictions(self):
        ...
        inner_preds_received = (
            self.hyperpipe.results_handler.get_validation_predictions()
        )
        first_outer_fold_info = next(
            iter(self.hyperpipe.cross_validation.outer_folds.values())
        )
        values_to_expect = np.asarray(first_outer_fold_info.train_indices) + 1.0
        self.assertTrue(
            np.array_equal(inner_preds_received["y_pred"], values_to_expect)
        )
        self.assertTrue(
            np.array_equal(inner_preds_received["y_true"], values_to_expect)
        )
        self.assertTrue(
            np.array_equal(inner_preds_received["probabilities"], values_to_expect / 10)
        )
        ...
        self.assertTrue(
            np.array_equal(outer_preds_received["probabilities"], self.y_true / 10)
        )
    ...
    def check_predictions_eval_final_performance_false(self):
        ...
        outer_fold_predictiosn_received = (
            self.hyperpipe.results_handler.get_test_predictions()
        )
        ...
        self.assertTrue(
            np.array_equal(csv_file.probabilities.values, values_to_expect / 10)
        )
    ...
    def test_best_config_stays_the_same(self):
        ...
        self.hyperpipe += PipelineElement("StandardScaler")
        self.hyperpipe += PipelineElement(
            "PCA", {"n_components": [4, 5]}, random_state=42
        )
    ...
    def test_metrics_and_aggregations(self):
        ...
        self.hyperpipe += PipelineElement(
            "PhotonTestXPredictor", change_predictions=True
        )
    ...
    def metric_assertions(self):
        ...
        for i, (_, outer_fold) in enumerate(
            ...
            for _, inner_fold in self.hyperpipe.cross_validation.inner_folds[
                ...
                expected_test_mae = mean_absolute_error(
                    XPredictor.adapt_X(global_test_indices), global_test_indices
                )
                ...
                self.assertTrue(
                    np.array_equal(
                        tree_result.validation.indices, inner_fold.test_indices
                    )
                )
                self.assertEqual(
                    len(global_test_indices), len(tree_result.validation.y_true)
                )
                self.assertEqual(
                    len(global_test_indices), len(tree_result.validation.y_pred)
                )
                ...
                expected_train_mae = mean_absolute_error(
                    XPredictor.adapt_X(global_train_indices), global_train_indices
                )
                ...
                self.assertTrue(
                    np.array_equal(
                        tree_result.training.indices, inner_fold.train_indices
                    )
                )
                self.assertEqual(
                    len(global_train_indices), len(tree_result.training.y_true)
                )
                self.assertEqual(
                    len(global_train_indices), len(tree_result.training.y_pred)
                )

                # get expected train and test mean and std respectively and calculate mean and std again.

            check_metrics(
                "mean_absolute_error", inner_fold_metrics["train"], config.metrics_train
            )
            check_metrics(
                "mean_absolute_error", inner_fold_metrics["test"], config.metrics_test
            )
            ...
            if self.hyperpipe.cross_validation.eval_final_performance:
                expected_outer_test_mae = mean_absolute_error(
                    XPredictor.adapt_X(outer_fold.test_indices), outer_fold.test_indices
                )

                self.assertTrue(
                    np.array_equal(
                        outer_fold_results.best_config.best_config_score.validation.indices,
                ...
                self.assertEqual(
                    ...
                    len(
                        outer_fold_results.best_config.best_config_score.validation.y_true
                    ),
                )
                self.assertEqual(
                    len(outer_fold.test_indices),
                    len(
                        outer_fold_results.best_config.best_config_score.validation.y_pred
                    ),
                )
                ...
                self.assertTrue(
                    np.array_equal(
                        outer_fold_results.best_config.best_config_score.training.indices,
                        outer_fold.train_indices,
                    )
                )
                self.assertEqual(
                    ...
                    len(
                        outer_fold_results.best_config.best_config_score.training.y_true
                    ),
                )
                self.assertEqual(
                    len(outer_fold.train_indices),
                    len(
                        outer_fold_results.best_config.best_config_score.training.y_pred
                    ),
                )
            else:
                ...
                self.assertTrue(
                    outer_fold_results.best_config.best_config_score.validation.metrics_copied_from_inner
                )
                self.assertTrue(
                    outer_fold_results.best_config.best_config_score.training.metrics_copied_from_inner
                )

            outer_collection["test"].append(expected_outer_test_mae)
            self.assertEqual(
                outer_fold_results.best_config.best_config_score.validation.metrics[
                    "mean_absolute_error"
                ],
                expected_outer_test_mae,
            )
            ...
            expected_outer_train_mae = mean_absolute_error(
                XPredictor.adapt_X(outer_fold.train_indices), outer_fold.train_indices
            )
            outer_collection["train"].append(expected_outer_train_mae)
            self.assertAlmostEqual(
                outer_fold_results.best_config.best_config_score.training.metrics[
                    "mean_absolute_error"
                ],
        ...
        self.assertListEqual(
            outer_fold_performances["mean_absolute_error"], outer_collection["test"]
        )
    ...
    def test_three_levels_of_feature_importances(self):
        ...
        for outer_fold in hyperpipe.results.outer_folds:
            self.assertTrue(
                len(outer_fold.best_config.best_config_score.feature_importances)
                == exepcted_nr_of_feature_importances
            )
