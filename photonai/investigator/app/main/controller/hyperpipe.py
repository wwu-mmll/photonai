...
def show_pipeline(storage, name):
    try:
        ...
        optimizer_history = plotly_optimizer_history(
            "optimizer_history",
            config_evaluations,
            min_config_evaluations,
            pipe.hyperpipe_info.best_config_metric,
        )
        plotly_dict = handler.eval_mean_time_components(
            write_results=False, plotly_return=True
        )
...
def create_performance_overview_plots_new(pipe):
    ...
    for metric in metrics:
        overview_plot = PlotlyPlot(
            "overview_plot_" + metric,
            metric.replace("_", " "),
            show_legend=False,
            margin={"r": 30, "l": 30},
        )
        for fold in pipe.outer_folds:
            overview_plot_train_trace = PlotlyTrace(
                "train_fold_" + str(fold.fold_nr),
                trace_color="train_color_bold",
                trace_size=10,
            )
            overview_plot_test_trace = PlotlyTrace(
                "test_fold_" + str(fold.fold_nr),
                trace_color="alternative_test_color_bold",
                trace_size=10,
            )
            ...
            if fold.best_config:
                ...
                overview_plot_train_trace.add_y(
                    fold.best_config.best_config_score.training.metrics[metric]
                )
                overview_plot_test_trace.add_x("test")
                overview_plot_test_trace.add_y(
                    fold.best_config.best_config_score.validation.metrics[metric]
                )
        ...
        training_mean_trace = PlotlyTrace(
            "mean_train",
            trace_size=4,
        ...
        test_dummy_trace = PlotlyTrace(
            "dummy_test",
            trace_size=4,
            trace_color="dummy_color",
            trace_type="bar",
            width=0.1,
            opacity=0,
        )
    ...
    for fold in pipe.outer_folds:
        ...
        if fold.best_config:
            ...
            for (
                key,
                value,
            ) in fold.best_config.best_config_score.training.metrics.items():
        ...
        metric_training_trace = BestConfigTrace(
            "training", metric_training_list, "", "bar"
        )
        metric_test_trace = BestConfigTrace("test", metric_validation_list, "", "bar")

        best_config_plot = BestConfigPlot(
            "outer_fold_" + str(fold.fold_nr) + "_best_config_overview",
            "Best Performance Outer Fold " + str(fold.fold_nr),
            metric_training_trace,
            metric_test_trace,
        )
...
def create_prediction_plot(handler):
    ...
    if handler.results.hyperpipe_info.eval_final_performance:
        for outer_fold in handler.results.outer_folds:
            outer_fold_values_val = handler.collect_fold_lists(
                [outer_fold.best_config.best_config_score.validation],
                [outer_fold.fold_nr],
            )
            true_and_pred_val.append(
                [outer_fold_values_val["y_true"], outer_fold_values_val["y_pred"]]
            )

            outer_fold_values_train = handler.collect_fold_lists(
                [outer_fold.best_config.best_config_score.training],
                [outer_fold.fold_nr],
            )
            true_and_pred_train.append(
                [outer_fold_values_train["y_true"], outer_fold_values_train["y_pred"]]
            )
    else:
        for inner_fold in handler.results.best_config.inner_folds:
            inner_fold_values_val = handler.collect_fold_lists(
                [inner_fold.validation], [inner_fold.fold_nr]
            )
            true_and_pred_val.append(
                [inner_fold_values_val["y_true"], inner_fold_values_val["y_pred"]]
            )
            inner_fold_values_train = handler.collect_fold_lists(
                [inner_fold.training], [inner_fold.fold_nr]
            )
            true_and_pred_train.append(
                [inner_fold_values_train["y_true"], inner_fold_values_train["y_pred"]]
            )

    predictions_plot_train = ""
    predictions_plot_test = ""
    if handler.results.hyperpipe_info.estimation_type == "regressor":
        predictions_plot_train = plot_scatter(
            true_and_pred_train, "predictions_plot_train", "True/Pred Training"
        )
        predictions_plot_test = plot_scatter(
            true_and_pred_val, "predictions_plot_test", "True/Pred Test"
        )
    else:
        predictions_plot_train = plotly_confusion_matrix(
            "predictions_plot_train", "Confusion Matrix Training", true_and_pred_train
        )
        predictions_plot_test = plotly_confusion_matrix(
            "predictions_plot_test", "Confusion Matrix Test", true_and_pred_val
        )
