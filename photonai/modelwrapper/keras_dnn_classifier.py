import numpy as np
from photonai.modelwrapper.keras_base_models import (
    KerasDnnBaseModel,
    KerasBaseClassifier,
)
...
class KerasDnnClassifier(KerasDnnBaseModel, KerasBaseClassifier):
    def __init__(
        ...
        super(KerasDnnClassifier, self).__init__(
    ...
    def multi_class(self, value):
        ...
        if not self.loss or self.loss in [
    ...
    def target_activation(self, value):
        if value == "sigmoid":
            ...
            raise ValueError(
                "The Classifcation subclass of KerasDnnBaseModel does not allow to use another "
    ...
    def loss(self, value):
        if value == "":
            ...
            self._multi_class
            and value
            in keras_dnn_base_model.get_loss_allocation()["multi_classification"]
        ):
            self._loss = value
        elif (
            (not self._multi_class)
            and value
            in keras_dnn_base_model.get_loss_allocation()["binary_classification"]
        ):
            self._loss = value
        else:
            raise ValueError(
                "Loss function is not supported. Feel free to use upperclass without restrictions."
            )
    ...
    def encode_targets(self, y):
        ...
        if not self.multi_class:
            if class_nums != 2 and self.loss == "binary_crossentropy":
                raise ValueError(
                    "Can not use binary classification with more or less than two target classes."
                )
