...
class OuterFoldTests(PhotonBaseTest):
    ...
    def prepare_and_fit(self, outer_fold_man=None):
        if outer_fold_man is None:
            ...
            outer_fold_man = OuterFoldManager(
                self.pipe,
    ...
    def test_current_best_config(self):
        def check_current_best_config_equality(outer_manager, fold_operation):
            ...
            self.assertEqual(
                ...
                str(
                    MDBHelper.get_metric(
                        outer_manager.current_best_config,
                        fold_operation,
                        self.optimization_info.best_config_metric,
                    )
                ),
        ...
        check_current_best_config_equality(
            outer_manager_across_folds, FoldOperations.RAW
        )
    ...
    def test_prepare(self):
        ...
        for ico, copied_object in enumerate(outer_fold_man.constraint_objects):
            self.assertIsNot(
                self.optimization_info.performance_constraints[ico], copied_object
            )
    ...
    def test_eval_final_performance(self):
        ...
        def case_check(outer_fold_man, operation_str, operation_type):
            ...
            best_config_score_vals = [
                "__".join([m_key, operation_str, str(m_val)])
                for m_key, m_val in outer_fold_man.result_object.best_config.best_config_score.validation.metrics.items()
            ]
            best_config_inner_cv_vals = [
                str(m)
                for m in outer_fold_man.result_object.best_config.metrics_test
            ...
            self.assertTrue(
                len(
                    outer_fold_man.result_object.best_config.best_config_score.validation.indices
                )
                == 0
            )
            self.assertTrue(
                len(
                    outer_fold_man.result_object.best_config.best_config_score.validation.y_pred
                )
                == 0
            )
    ...
    def test_save_predictions_and_feature_importances(self):
        ...
        np.random.seed(42)
        outer_fold_man1 = OuterFoldManager(
            self.pipe,
            self.optimization_info,
            self.outer_fold_id,
            self.cv_info,
        ...
        self.assertTrue(
            len(best_config.best_config_score.validation.y_true)
            == len(self.cv_info.outer_folds[self.outer_fold_id].test_indices)
        )
        self.assertTrue(
            len(best_config.best_config_score.validation.y_pred)
            == len(self.cv_info.outer_folds[self.outer_fold_id].test_indices)
        )
        ...
        for config in outer_fold_man1.result_object.tested_config_list:
            if config.config_nr == best_config.config_nr:
                ...
                self.assertTrue(
                    np.sum(len(fold.validation.y_pred) for fold in config.inner_folds)
                    == 0
                )
                self.assertTrue(
                    np.sum(len(fold.feature_importances) for fold in config.inner_folds)
                    == 0
                )
    ...
    def test_find_best_config_always_again(self):
        ...
        self.assertDictEqual(
            outer_fold_man1.result_object.best_config.config_dict,
        ...
        self.assertListEqual(
            [str(m) for m in outer_fold_man1.result_object.best_config.metrics_train],
            [str(m) for m in outer_fold_man2.result_object.best_config.metrics_train],
        )

        self.assertListEqual(
            [str(m) for m in outer_fold_man1.result_object.best_config.metrics_test],
            [str(m) for m in outer_fold_man2.result_object.best_config.metrics_test],
        )

        self.assertDictEqual(
            outer_fold_man1.result_object.best_config.best_config_score.validation.metrics,
            outer_fold_man2.result_object.best_config.best_config_score.validation.metrics,
        )

        self.assertDictEqual(
            outer_fold_man1.result_object.best_config.best_config_score.training.metrics,
            outer_fold_man2.result_object.best_config.best_config_score.training.metrics,
        )

    def test_fit_dummy(self):
        self.optimization_info.performance_constraints = DummyPerformance(
            self.optimization_info.best_config_metric
        )
        outer_fold_man = OuterFoldManager(
            self.pipe,
        ...
        outer_fold_man._prepare_data(
            np.ones((self.X.shape[0], self.X.shape[1], 1)), self.y
        )
        outer_fold_man._fit_dummy()
        self.assertIsNone(outer_fold_man.result_object.dummy_results)
        ...
        train_values = {
            "mean_absolute_error": 6.809283403587883,
            "mean_squared_error": 86.87340383295755,
        }
        test_values = {
            "mean_absolute_error": 6.255843525529023,
            "mean_squared_error": 75.04543037399255,
        }

        self.assertDictEqual(
            outer_fold_man.result_object.dummy_results.validation.metrics, test_values
        )
        self.assertDictEqual(
            outer_fold_man.result_object.dummy_results.training.metrics, train_values
        )

        # check that performance constraints are updated
        self.assertTrue(
            outer_fold_man.constraint_objects[0].threshold
            == outer_fold_man.result_object.dummy_results.validation.metrics[
                self.optimization_info.best_config_metric
            ]
        )
