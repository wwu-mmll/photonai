...
class GridSearchOptimizerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.pipeline_elements = [
            ...
            PipelineElement(
                "PCA", hyperparameters={"n_components": IntegerRange(5, 20)}
            ),
            PipelineElement("SVC"),
        ]
    ...
    def test_all_functions_available(self):
        ...
        self.assertListEqual(
            list(signature(self.optimizer.prepare).parameters.keys()),
            ["pipeline_elements", "maximize_metric"],
        )
        self.assertTrue(hasattr(self.optimizer, "tell"))
        self.assertListEqual(
            list(signature(self.optimizer.tell).parameters.keys()),
            ["config", "performance"],
        )
    ...
    def test_all_attributes_available(self):
        ...
        self.optimizer.prepare(
            pipeline_elements=self.pipeline_elements, maximize_metric=True
        )
    ...
    def test_ask(self):
        ...
        self.assertSetEqual(
            set([str(type(a)) for a in ask_list]), set(["<class 'dict'>"])
        )
    ...
    def test_ask_advanced(self):
        ...
        pipe_switch = Switch(
            "switch",
            [PipelineElement("StandardScaler"), PipelineElement("MaxAbsScaler")],
        )
        self.pipeline_elements = [
            PipelineElement("StandardScaler"),
            PipelineElement(
