...
from photonai.test.base_tests.dummy_elements import (
    DummyEstimator,
    DummyNeedsCovariatesEstimator,
    DummyNeedsCovariatesTransformer,
    DummyNeedsYTransformer,
    DummyTransformer,
    DummyNeedsCovariatesAndYTransformer,
    DummyEstimatorNoPredict,
    DummyEstimatorWrongType,
...
class PipelineElementTests(unittest.TestCase):
    ...
    def test_fit(self):
        ...
        self.assertEqual(
            self.pca_pipe_element.base_element.components_[0, 0], 0.005086232018734175
        )

        self.svc_pipe_element.fit(self.X, self.y)
        self.assertEqual(
            self.svc_pipe_element.base_element._intercept_, -0.3753900173819406
        )
    ...
    def test_one_hyperparameter_setup(self):
        ...
        self.assertDictEqual(
            self.pca_pipe_element.hyperparameters,
            {"PCA__n_components": [1, 2], "PCA__disabled": [False, True]},
        )

        # config_grid is created as expected
        self.assertListEqual(
            self.pca_pipe_element.generate_config_grid(),
            [
    ...
    def test_more_hyperparameters_setup(self):
        # sklearn attributes are generated
        self.assertDictEqual(
            self.svc_pipe_element.hyperparameters,
            {"SVC__C": [0.1, 1], "SVC__kernel": ["rbf", "sigmoid"]},
        )

        # config_grid is created as expected
        self.assertListEqual(
            self.svc_pipe_element.generate_config_grid(),
            [
    ...
    def test_no_hyperparameters(self):
        ...
        self.assertDictEqual(
            pca_sklearn_element.__dict__, pca_photon_element.base_element.__dict__
        )
    ...
    def test_adjusted_delegate_call_transformer(self):
        ...
        trans = PipelineElement.create(
            "Transformer", base_element=DummyTransformer(), hyperparameters={}
        )
    ...
    def test_adjusted_delegate_call_estimator(self):
        ...
        est = PipelineElement.create(
            "Estimator", base_element=DummyEstimator(), hyperparameters={}
        )
        y = est.predict(self.X)
        self.assertTrue(
            np.array_equal(y, self.Xt)
        )  # DummyEstimator returns X as y predictions
...
class SwitchTests(unittest.TestCase):
    ...
    def test_hyperparams(self):
        ...
        self.assertDictEqual(
            ...
            {
                "estimator_switch__current_element": [
                    (0, 0),
                    (0, 1),
                    (0, 2),
                    (0, 3),
        ...
        # config grid
        self.assertListEqual(
            self.estimator_switch.generate_config_grid(),
            [
    ...
    def test_copy_me(self):
        switches = [
            self.estimator_switch,
            self.estimator_switch_with_branch,
    ...
    def test_estimator_type(self):
        ...
        self.assertEqual(
            self.estimator_switch_with_branch._estimator_type, "classifier"
        )
    ...
    def test_add(self):
        ...
        switch = Switch(
            "MySwitch", [PipelineElement("PCA"), PipelineElement("FastICA")]
        )
        ...
        self.assertEqual(
            self.estimator_switch.pipeline_element_configurations[4][0]["SVC3__kernel"],
            "polynomial",
        )
...
class BranchTests(unittest.TestCase):
    def setUp(self):
        ...
        self.pca = PipelineElement(
            "PCA", {"n_components": [1, 2]}, test_disabled=True, random_state=3
        )
        self.tree = PipelineElement(
            "DecisionTreeClassifier", {"min_samples_split": [2, 3, 4]}, random_state=3
        )

        self.transformer_branch = Branch("MyBranch", [self.scaler, self.pca])
        self.transformer_branch_sklearn = SKPipeline(
    ...
    def test_transform(self):
        ...
        Xt_sklearn = self.transformer_branch_sklearn.fit(self.X, self.y).transform(
            self.X
        )
    ...
    def test_predict(self):
        ...
        y_pred_sklearn = self.estimator_branch_sklearn.fit(self.X, self.y).predict(
            self.X
        )
    ...
    def test_predict_proba(self):
        ...
        proba_sklearn = self.estimator_branch_sklearn.fit(self.X, self.y).predict_proba(
            self.X
        )
    ...
    def test_inverse_transform(self):
        ...
        feature_importances = self.estimator_branch.elements[
            -1
        ].base_element.feature_importances_
    ...
    def test_copy_me(self):
        ...
        self.assertNotEqual(
            copy.elements[1].base_element.n_components,
        ...
        self.assertEqual(
            fake_copy.elements[1].base_element.n_components,
            branch.elements[1].base_element.n_components,
        )
    ...
    def test_sanity_check_pipe(self):
        ...
        with self.assertRaises(Warning):
            my_callback = CallbackElement(
                "final_element_callback", delegate_function=callback_func
            )
    ...
    def test_set_params(self):
        ...
        self.assertEqual(
            self.transformer_branch.base_element.elements[0][1].base_element.with_mean,
            True,
        )
    ...
    def test_estimator_type(self):
        ...
        transformer_branch = Branch(
            "TransBranch", [PipelineElement("PCA"), PipelineElement("FastICA")]
        )
        ...
        callback_branch = Branch(
            "CallBranch",
            [PipelineElement("SVR"), CallbackElement("callback", callback)],
        )
    ...
    def test_add(self):
        branch = Branch(
            "MyBranch",
            [PipelineElement("PCA", {"n_components": [5]}), PipelineElement("FastICA")],
        )
        self.assertEqual(len(branch.elements), 2)
        self.assertDictEqual(
            branch._hyperparameters, {"MyBranch__PCA__n_components": [5]}
        )
...
class StackTests(unittest.TestCase):
    def setUp(self):
        ...
        self.tree = PipelineElement(
            "DecisionTreeClassifier", {"min_samples_leaf": [3, 5]}
        )
        ...
        self.transformer_stack = Stack(
            "TransformerStack", [self.pca.copy_me(), self.scaler.copy_me()]
        )
        self.estimator_stack = Stack(
            "EstimatorStack", [self.svc.copy_me(), self.tree.copy_me()]
        )
        self.transformer_branch_stack = Stack(
            "TransBranchStack",
            [self.transformer_branch_1.copy_me(), self.transformer_branch_2.copy_me()],
        )
        self.estimator_branch_stack = Stack(
            "EstBranchStack",
            [self.estimator_branch_1.copy_me(), self.estimator_branch_2.copy_me()],
        )
        ...
        self.stacks = [
            ...
            (
                [self.transformer_branch_1, self.transformer_branch_2],
                self.transformer_branch_stack,
            ),
            (
                [self.estimator_branch_1, self.estimator_branch_2],
                self.estimator_branch_stack,
            ),
        ]
    ...
    def test_set_params(self):
        ...
        est_config = {"SVC__C": 3, "DecisionTreeClassifier__min_samples_leaf": 1}
    ...
    def test_add(self):
        ...
        self.assertDictEqual(
            stack._hyperparameters, {"MyStack__PCA__n_components": [5]}
        )
        stack = Stack("MyStack")
...
class CallbackElementTests(unittest.TestCase):
    def setUp(self):
        ...
        self.callback_pipeline = PhotonPipeline(
            elements=[
                ("First", CallbackElement("First", callback)),
                ("PCA", PipelineElement("PCA")),
        ...
        self.clean_branch_pipeline = PhotonPipeline(
            elements=[
                ("MyBranch", Branch("MyBranch", [PipelineElement("PCA")])),
                ("LogisticRegression", PipelineElement("LogisticRegression")),
            ]
        )
        self.callback_branch_pipeline = PhotonPipeline(
            elements=[
                ...
                (
                    ...
                    Branch(
                        "MyBranch",
                        [CallbackElement("Second", callback), PipelineElement("PCA")],
                    ),
                ),
    ...
    def test_callback(self):
        pipelines = [
            self.clean_pipeline,
            self.callback_pipeline,
            self.clean_branch_pipeline,
            self.callback_branch_pipeline,
            self.callback_after_callback_pipeline,
        ]
